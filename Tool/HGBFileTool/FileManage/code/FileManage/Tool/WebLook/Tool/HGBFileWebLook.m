...
-(void)lookFileAtSource:(NSString *)source inParent:(UIViewController *)parent{
    if(parent==nil){
        parent=[HGBFileWebLook currentViewController];

    }
    if(source==nil&&source.length==0){
        [self alertWithPrompt:@"url不能为空"];
        if (self.delegate&&[self.delegate respondsToSelector:@selector(webLookDidOpenFailed:)]) {
            [self.delegate webLookDidOpenFailed:self];
        }
        HGBLog(@"url不能为空");
        return;
    }
    source=[HGBFileWebLook urlAnalysis:source];
    if(![HGBFileWebLook urlExistCheck:source]){
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(webLookDidOpenSucessed:)]) {
        [self.delegate webLookDidOpenSucessed:self];
    }
     self.webView=[[HGBFileWebController alloc]init];

    [self.webView createNavigationItemWithTitle:[source lastPathComponent]];
     [self.webView loadHtmlSource:source];

    UINavigationController *nav=[[UINavigationController alloc]initWithRootViewController:self.webView];
...
+(NSString *)urlAnalysisToPath:(NSString *)url{
    ...
    NSString *urlstr=[HGBFileWebLook urlAnalysis:url];
