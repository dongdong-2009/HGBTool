...
/**
 ...
 @param delegate 代理
 */
+(void)setQuickLookDelegate:(id<HGBFileOutAppOpenFileToolDelegate>)delegate{
    [HGBFileOutAppOpenFileTool shareInstance];
...
-(void)lookFileAtSource:(NSString *)source inParent:(UIViewController *)parent{
    ...
    if(parent==nil){
        parent=[HGBFileOutAppOpenFileTool currentViewController];

    }
    if(source==nil&&source.length==0){
        if (self.delegate&&[self.delegate respondsToSelector:@selector(outLook:didOpenFailedWithError:)]) {
            [self.delegate outLook:self didOpenFailedWithError:@{ReslutCode:@(HGBFileOutAppOpenFileToolErrorTypePath).stringValue,ReslutMessage:@"url不能为空"}];
        }
        HGBLog(@"url不能为空");
    ...
    source=[HGBFileOutAppOpenFileTool urlAnalysis:source];

    if(![HGBFileOutAppOpenFileTool urlExistCheck:source]){
        if (self.delegate&&[self.delegate respondsToSelector:@selector(outLook:didOpenFailedWithError:)]) {
            [self.delegate outLook:self didOpenFailedWithError:@{ReslutCode:@(HGBFileOutAppOpenFileToolErrorTypePath).stringValue,ReslutMessage:@"url无法打开"}];
        }
        HGBLog(@"url无法发开");
...
+(NSString *)urlAnalysisToPath:(NSString *)url{
    ...
    NSString *urlstr=[HGBFileOutAppOpenFileTool urlAnalysis:url];
...
-(void)alertWithPrompt:(NSString *)prompt{
    ...
    [[HGBFileOutAppOpenFileTool currentViewController] presentViewController:alert animated:YES completion:nil];
#else
...
+ (UIViewController *)currentViewController {
    ...
    return [HGBFileOutAppOpenFileTool findBestViewController:viewController];
}
...
+ (UIViewController *)findBestViewController:(UIViewController *)vc
{
    if (vc.presentedViewController) {
        ...
        return [HGBFileOutAppOpenFileTool findBestViewController:vc.presentedViewController];
    } else if ([vc isKindOfClass:[UISplitViewController class]]) {
        // Return right hand side
        UISplitViewController *svc = (UISplitViewController *) vc;
        if (svc.viewControllers.count > 0){
            return [HGBFileOutAppOpenFileTool findBestViewController:svc.viewControllers.lastObject];
        }else{
            return vc;
        }
        ...
        if (svc.viewControllers.count > 0){
            return [HGBFileOutAppOpenFileTool findBestViewController:svc.topViewController];
        }else{
            return vc;
        }
        ...
        if (svc.viewControllers.count > 0){
            return [HGBFileOutAppOpenFileTool findBestViewController:svc.selectedViewController];
        }else{
            return vc;
        }
    } else {
