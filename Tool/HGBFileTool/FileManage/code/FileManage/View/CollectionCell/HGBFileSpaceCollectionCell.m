...
@interface HGBFileSpaceCollectionCell()
/**
 图片按钮
 */
...
@end
@implementation HGBFileSpaceCollectionCell
#pragma mark init
...
-(void)viewSetUp{
    ...
    self.imageView=[[UIImageView alloc]initWithFrame:CGRectMake(kWidth*0.05, kWidth*0.05, kWidth*0.15, kWidth*0.15)];
    ...
    self.titleLabel=[[UILabel alloc]initWithFrame:CGRectMake(kWidth*0.025, kWidth*0.2, kWidth*0.2, kWidth*0.1)];
...
-(void)layoutSubviews{
    self.imageView.frame=CGRectMake(kWidth*0.05, kWidth*0.05, kWidth*0.15, kWidth*0.15);
    self.titleLabel.frame=CGRectMake(kWidth*0.025, kWidth*0.2, kWidth*0.2, kWidth*0.1);
    self.imageButton.frame=self.imageView.frame;
//     self.selectButton.frame=CGRectMake(self.frame.size.width-46*wScale, 10*hScale, 36*wScale, 36*wScale);
...
-(void)imageButtonHandle:(UIButton *)_b{
    if(self.delegate&&[self.delegate respondsToSelector:@selector(fileSpaceCollectionCell:didClickImageWithIndexPath:)]){
        [self.delegate fileSpaceCollectionCell:self didClickImageWithIndexPath:self.indexPath];
    }
}
#pragma mark action
-(void)longPressHandler:(UILongPressGestureRecognizer *)_p{
//      self.isSelect=YES;
    if(self.delegate&&[self.delegate respondsToSelector:@selector(fileSpaceCollectionCell:didLongPressImageWithIndexPath:)]){
        [self.delegate fileSpaceCollectionCell:self didLongPressImageWithIndexPath:self.indexPath];
    }
}
@end
