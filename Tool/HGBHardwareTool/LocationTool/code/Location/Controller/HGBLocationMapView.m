...
//        CLPlacemark *placemark = [placemarks firstObject];
        ...
        if(error){
            ...
            if(self.delegate&&[self.delegate respondsToSelector:@selector(mapview:didFailedWithError:)]){
                [self.delegate mapview:self didFailedWithError:@{ReslutCode:@(HGBLocationMapViewErrorTypeLocationError).stringValue,ReslutMessage:error.localizedDescription}];
            }
            return;
        }
        ...
        if(locationMarks.count==0){
            HGBLog(@"目标地址不存在");
            if(self.delegate&&[self.delegate respondsToSelector:@selector(mapview:didFailedWithError:)]){
                [self.delegate mapview:self didFailedWithError:@{ReslutCode:@(HGBLocationMapViewErrorTypeLocationError).stringValue,ReslutMessage:@"目标地址不存在"}];
            }
...
-(void)mapViewDidFailLoadingMap:(MKMapView *)mapView withError:(NSError *)error{
    ...
    HGBLog(@"定位失败");
    if(self.delegate&&[self.delegate respondsToSelector:@selector(mapview:didFailedWithError:)]){
        [self.delegate mapview:self didFailedWithError:@{ReslutCode:@(HGBLocationMapViewErrorTypeLocationError).stringValue,ReslutMessage:@"定位失败"}];
    }
}
#pragma mark 权限
-(void)setAuthority{
