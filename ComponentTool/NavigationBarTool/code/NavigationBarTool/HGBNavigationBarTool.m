...
@implementation HGBNavigationBarTool
#pragma mark 导航栏整体
/**
 设置当前导航栏显示隐藏
...
+(void)setNavgationBarIsHidden:(BOOL)isHidden{
    UIViewController *controller=[HGBNavigationBarTool currentViewController];
...
+(void)createNavigationItem:(HGBNavigationItem )type andWithTitles:(NSArray <NSString *>*)titles andWithTarget:(id)target andWithAction:(SEL)action inController:(UIViewController *)controller{
    if(type==HGBNavigationItemTitle){
        [HGBNavigationBarTool createNavigationItemTitleWithTitles:titles andWithTarget:target andWithAction:action inController:controller];
    }else if(type==HGBNavigationItemLeft){
        [HGBNavigationBarTool createNavigationItemLeftWithTitles:titles andWithTarget:target andWithAction:action inController:controller];
    }else{
        [HGBNavigationBarTool createNavigationItemRightWithTitles:titles andWithTarget:target andWithAction:action inController:controller];
    }
}
/**
 创建导航栏-标题按钮多个采用segment
...
+(void)createNavigationItemWithTitles:(NSDictionary *)titles andWithTarget:(id)target andWithAction:(SEL )action inController:(UIViewController *)controller{
    ...
    if(titleArray&&[titleArray isKindOfClass:[NSArray class]]){
        [HGBNavigationBarTool createNavigationItemTitleWithTitles:titleArray andWithTarget:target andWithAction:action inController:controller];
    }
    if(titleArray&&[titleArray isKindOfClass:[NSArray class]]){
        [HGBNavigationBarTool createNavigationItemLeftWithTitles:leftArray andWithTarget:target andWithAction:action inController:controller];
    }
    if(rightArray&&[rightArray isKindOfClass:[NSArray class]]){
        [HGBNavigationBarTool createNavigationItemRightWithTitles:rightArray andWithTarget:target andWithAction:action inController:controller];
    }
}
...
+ (UIViewController *)currentViewController {
    ...
    return [HGBNavigationBarTool findBestViewController:viewController];
}
/**
 寻找上层控制器
...
+ (UIViewController *)findBestViewController:(UIViewController *)vc
{
    if (vc.presentedViewController) {
        ...
        return [HGBNavigationBarTool findBestViewController:vc.presentedViewController];
    } else if ([vc isKindOfClass:[UISplitViewController class]]) {
        // Return right hand side
        UISplitViewController *svc = (UISplitViewController *) vc;
        if (svc.viewControllers.count > 0){
            return [HGBNavigationBarTool findBestViewController:svc.viewControllers.lastObject];
        }else{
            return vc;
        }
        ...
        if (svc.viewControllers.count > 0){
            return [HGBNavigationBarTool findBestViewController:svc.topViewController];
        }else{
            return vc;
        }
        ...
        if (svc.viewControllers.count > 0){
            return [HGBNavigationBarTool findBestViewController:svc.selectedViewController];
        }else{
            return vc;
        }
    } else {
